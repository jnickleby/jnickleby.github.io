...
RuneScapeComponent.ctorParameters = () => [
...
let RuneWallComponent = class RuneWallComponent {
    constructor(runeService) {
        this.runeService = runeService;
    }
    ngOnInit() {
...
RuneWallComponent.ctorParameters = () => [
...
let TabletButtonComponent = class TabletButtonComponent {
    constructor(runeService) {
        this.runeService = runeService;
        this.isGlyph = false;
        this.rounded = true;
...
TabletButtonComponent.ctorParameters = () => [
    { type: _services_rune_service__WEBPACK_IMPORTED_MODULE_2__["RuneService"] }
];
...
class Rune {
    constructor(expression, name, description, parentGroup, atomicGroup, atomic = false, overview) {
...
const BAN = new Rune('>', 'ban', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const CAB = new Rune('_', 'cab', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const COM = new Rune(',', 'com', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const FAS = new Rune('/', 'fas', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const HEP = new Rune('-', 'hep', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const LED = new Rune('<', 'led', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const NET = new Rune('/', 'net', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
const PAD = new Rune('&', 'pad', 'Used to make glyphs', NOT, 'Glyph Combinator', true);
...
// region KEY
const KETBAR = KET.combineRune(BAR, 'Converts a gold core to an iron core');
const KETCOL = KET.combineRune(COL, 'Creates a factory gate for a specified type');
const KETDOT = KET.combineRune(DOT, 'Performs a typecast on value produces from p to q');
const KETHEP = KET.combineRune(HEP, 'Performs a typecast by an explicit type label');
const KETLUS = KET.combineRune(LUS, 'Perform typecast by inferred type');
const KETPAD = KET.combineRune(PAD, 'Converts a core to a zinc core (covariant)');
const KETSIG = KET.combineRune(SIG, 'Folds input as constant at compile time if possible');
const KETTAR = KET.combineRune(TAR, 'Produces an example type value');
const KETTIS = KET.combineRune(TIS, 'Binds a name to a value');
const KETWUT = KET.combineRune(WUT, 'Converts any core to a lead core (bivariant)');
// endregion KEY
const KET_SET = {
    parent: KET,
...
// region LUS
const LUSBAR = LUS.combineRune(BAR, 'Creates a chapter label');
const LUSBUC = LUS.combineRune(BUC, 'Produces a structure arm (type definition)');
const LUSLUS = LUS.combineRune(LUS, 'Produces a normal arm');
const LUSTAR = LUS.combineRune(TAR, 'Produces a type constructor arm');
// endregion LUS
const LUS_SET = {
    parent: LUS,
...
// region MIC
const MICCOL = MIC.combineRune(COL, 'Calls a binary function as an n-ary function');
const MICLUS = MIC.combineRune(LUS, 'Makes a single XML node (Sail)');
const MICMIC = MIC.combineRune(MIC, 'Normalizes with a mold, asserting fixpoint');
const MICFAS = MIC.combineRune(FAS, 'Tape as XML element');
const MICSIG = MIC.combineRune(SIG, 'Glues a pipeline together with a product-sample adapter');
const MICTAR = MIC.combineRune(TAR, 'Makes a list of XML nodes from complex Hoon expression (Sail)');
const MICTIS = MIC.combineRune(TIS, 'Makes a list of XML nodes (Sail)');
// endregion MIC
const MIC_SET = {
    parent: MIC,
...
// region SIG
const SIGBAN = SIG.combineRune(BAN, 'Creates a raw hint applied to computation');
const SIGBAR = SIG.combineRune(BAR, 'Creates a tracing printf');
const SIGBUC = SIG.combineRune(BUC, 'Creates a profiling hit counter');
const SIGCAB = SIG.combineRune(CAB, 'Creates a user-formatted tracing printf');
const SIGCEN = SIG.combineRune(CEN, 'Creates a jet registration');
const SIGLED = SIG.combineRune(LED, 'Creates a raw hint applied to product');
const SIGLUS = SIG.combineRune(LUS, 'Caches a computation');
const SIGNET = SIG.combineRune(NET, 'Creates a jet registration for gate with registered context');
const SIGPAD = SIG.combineRune(PAD, 'Creates a debugging printf');
const SIGTIS = SIG.combineRune(TIS, 'Detects duplicate');
const SIGWUT = SIG.combineRune(WUT, 'Creates a conditional debug printf');
const SIGZAP = SIG.combineRune(ZAP, 'Sets print type on compilation fail');
// endregion SIG
const SIG_SET = {
    parent: SIG,
...
// region TIS
const TISBAN = TIS.combineRune(BAN, 'Composes two expressions');
const TISBAR = TIS.combineRune(BAR, 'Combines a default type value with the subject');
const TISCOL = TIS.combineRune(COL, 'Changes multiple legs in the subject');
const TISCOM = TIS.combineRune(COM, 'Exposes a namespace');
const TISDOT = TIS.combineRune(DOT, 'Changes one leg in the subject');
const TISFAS = TIS.combineRune(FAS, 'Combines a named noun with the subject, possibly with type annotation');
const TISHEP = TIS.combineRune(HEP, 'Combines a new noun with the subject, inverted');
const TISKET = TIS.combineRune(KET, 'Pins the head of a pair and changes a leg with a tail');
const TISLED = TIS.combineRune(LED, 'Composes two expressions, inverted');
const TISLUS = TIS.combineRune(LUS, 'Combines a new noun with the subject');
const TISMIC = TIS.combineRune(MIC, 'Combines a named noun with the subject, possibly with type annotation; inverted order');
const TISSIG = TIS.combineRune(SIG, 'Composes many expressions');
const TISTAR = TIS.combineRune(TAR, 'Defines an alias');
const TISWUT = TIS.combineRune(WUT, 'Conditionally changes one leg in the subject');
// endregion TIS
const TIS_SET = {
    parent: TIS,
...
// region WUT
const WUTBAN = WUT.combineRune(BAN, 'Creates a positive assertion');
const WUTBAR = WUT.combineRune(BAR, 'Creates logical OR');
const WUTCOL = WUT.combineRune(COL, 'Creates a branch on a boolean test');
const WUTDOT = WUT.combineRune(DOT, 'Branches on a boolean test, inverted');
const WUTHEP = WUT.combineRune(HEP, 'Switches against a union, with no default');
const WUTKET = WUT.combineRune(KET, 'Branches on whether a wing of the subject evaluates as a cell');
const WUTLED = WUT.combineRune(LED, 'Creates a negative assertion');
const WUTLUS = WUT.combineRune(LUS, 'Switches against a union, with a default');
const WUTPAD = WUT.combineRune(PAD, 'Creates logical AND');
const WUTSIG = WUT.combineRune(SIG, 'Branches on whether a wing of the subject evaluates to null');
const WUTTIS = WUT.combineRune(TIS, 'Tests pattern match');
const WUTVAT = WUT.combineRune(VAT, 'Branches on whether a wing of the subject evaluates to an atom');
const WUTZAP = WUT.combineRune(ZAP, 'Creates logical NOT');
// endregion WUT
const WUT_SET = {
    parent: WUT,
...
// region ZAP
const ZAPBAN = ZAP.combineRune(BAN, 'Wraps a noun in its type');
const ZAPCOL = ZAP.combineRune(COL, 'Turns on stack trace');
const ZAPDOT = ZAP.combineRune(DOT, 'Turns off stack trace for a subexpression p');
const ZAPTIS = ZAP.combineRune(TIS, 'Makes the Nock formula for a Hoon expression');
const ZAPWUT = ZAP.combineRune(WUT, 'Restricts Hoon version');
const ZAPZAP = ZAP.combineRune(ZAP, 'Crashes program');
// endregion ZAP
const ZAP_SET = {
    parent: ZAP,
...
let RuneService = class RuneService {
    constructor() {
